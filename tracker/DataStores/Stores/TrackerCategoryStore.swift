...
final class TrackerCategoryStore: NSObject {
    ...
    init(day: WeekDay, date: Date ) {
        ...
        let conroller = NSFetchedResultsController(
    ...
    func newCategory(categoryName: String, trackers: [Tracker] = []) {
        ...
        if trackers.isEmpty {
            ...
            let trackersCD = trackers.compactMap {
                trackerStore.getTrackerCD(
                    from: $0,
        ...
        request.predicate = NSPredicate(format: "title == %@", categoryName)
         do {
            ...
            guard let trackerStore = self.trackerStore,
                  let trackersCD = trackerStore.getTrackerCD(
                    from: tracker,
                    categoryName: categoryName
                  ) else { fatalError() }
...
extension TrackerCategoryStore: NSFetchedResultsControllerDelegate {
    ...
    func controllerDidChangeContent(_ controller: NSFetchedResultsController<any NSFetchRequestResult>) {
        delegate?.stote(
            self,
            didUpdate: TrackerCategoryStoreUpdate(insertedCategoryIndexes: insertedIndexes, updatedCategoryIndexes: updatedIndexes)
        )
        insertedIndexes = []
        updatedIndexes = []
    }
...
extension TrackerCategoryStore: TrackerCategoryStoreProtocol {
    ...
    func getCategoryCountByIndex(index: Int) -> Int {
        switch filter {
        case .all:
            return filteredCategories[index].trackerList.filter( {
                $0.timetable.contains(self.day)
            }).count
        case .today:
            return filteredCategories[index].trackerList.filter( {
                $0.timetable.contains(self.day)
        ...
        case .completed:
            return filteredCategories[index].trackerList.filter( {
                trackerRecord?.isRecordedByDate(id: $0.id, date: date) == true && $0.timetable.contains(self.day)
            }).count
        case .uncompleted:
            return filteredCategories[index].trackerList.filter( {
                trackerRecord?.isRecordedByDate(id: $0.id, date: date) == false && $0.timetable.contains(self.day)
            }).count
        }
    }
    func getTrackerByIndexPath(index: IndexPath) -> Tracker {
        ...
        case .today:
            return filteredCategories[index.section].trackerList[index.row]
        case .completed:
            return filteredCategories[index.section].trackerList.filter( {
                trackerRecord?.isRecordedByDate(id: $0.id, date: date) == true
            })[index.row]
        case .uncompleted:
            return filteredCategories[index.section].trackerList.filter( {
                trackerRecord?.isRecordedByDate(id: $0.id, date: date) == false
            })[index.row]
        }
    }
    ...
    func isPinnedById(trackerId: UUID) -> Bool {
        guard let category = categories.first(where: {
            $0.title == NSLocalizedString("category_pinned", comment: "")
        }) else { return false }
